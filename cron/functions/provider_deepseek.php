 ...
 * DeepSeek определяет вероятность нахождения товара на каждой странице по шкале 0–10
 * и возвращает JSON с execution_status=1.
 *
...
Правила:
- Оцени вероятность нахождения данного товара на каждой странице по шкале от 0 до 10:
  - 10 — товар точно продаётся на этой странице
  - 7–9 — высокая вероятность (каталог/категория с таким товаром)
  - 4–6 — средняя вероятность (страница может содержать товар)
  - 1–3 — низкая вероятность (скорее всего товара нет)
  - 0 — точно не относится к товару (сервисы, контакты, новости и т.п.)
...
[{"id": число, "html": "строка", "execution_status": 1, "priority": число от 0 до 10}]
PROMPT;
...
function classifyLinksWithDeepseek(string $deepseekKey, string $productInfo, array $links, int $maxRetries = 3): array
{
    ...
    for ($attempt = 1; $attempt <= $maxRetries; $attempt++) {
        ...
        // При ошибке таймаута или временных ошибок сервера — повторить попытку
        ...
        if ($isRetryable && $attempt < $maxRetries) {
            echo "⏳ Попытка {$attempt}/{$maxRetries} не удалась ({$errorMsg}), повтор через 5 сек...\n";
            sleep(5);
            continue;
        }
