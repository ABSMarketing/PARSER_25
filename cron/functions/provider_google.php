...
function getAllDataFromGoogleSheets($apiKey, $userId, $provider, $sheetName = 'Платформы') {
    ...
    if (isset($sheetData['status'])) {
        ...
        // Проверяем, что Google вернул данные именно запрошенной вкладки
        if (isset($sheetData['sheet_name']) && $sheetData['sheet_name'] !== $sheetName) {
            error_log("[Google Sheets] ПРЕДУПРЕЖДЕНИЕ: запрошена вкладка '$sheetName', но получена '{$sheetData['sheet_name']}'. Возможно кэширование.");
        }
