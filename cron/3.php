 ...
 *  3. По id найденной записи получаем фиксированный батч (LIMIT) необработанных ссылок
 *     из parsed_links (фильтр execution_status IS NULL, сортировка id ASC).
 *     Если необработанных ссылок нет — ставим status=2 и выходим.
 ...
 *  6. Сохраняем результат классификации в parsed_links.
 *  7. Если батч неполный (ссылок меньше лимита) — ставим status=2 (все ссылки обработаны).
 *     Если батч полный — оставляем status=1, обновляем updated_at; при следующем запуске
 *     крона будет обработан следующий батч.
 *
 * Скрипт рассчитан на многократный запуск по крону для одного товара.
 *
...
$batchLimit = 10; // фиксированное количество ссылок за один заход крона

$links = getLinksByParentId($pdo, $productId, $batchLimit);

// Флаг: последний батч — ссылок вернулось меньше лимита, значит необработанных больше нет
$isLastBatch = (count($links) < $batchLimit);

if (empty($links)) {
    echo "ℹ️  Нет необработанных ссылок.\n";
    updateProductStatus($pdo, $productId, true);
    exit(0);
}

echo "📊 Найдено необработанных ссылок: " . count($links) . "\n\n";
...
// ========================================
if ($saved && $isLastBatch) {
    // Все необработанные ссылки исчерпаны → финализируем товар
    updateProductStatus($pdo, $productId, true);
    echo "✅ Все ссылки обработаны, статус → 2\n\n";
} elseif ($saved) {
    // Батч полный, ещё есть необработанные ссылки → оставляем status=1
    updateProductStatus($pdo, $productId, false);
    echo "⏳ Батч обработан (" . count($links) . " ссылок), остались ещё → status остаётся 1\n\n";
} else {
    updateProductStatus($pdo, $productId, false);
